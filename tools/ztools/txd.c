...
#define ROUND_DATA(address)                                                    \
    ((address + (story_scaler - 1)) & ~(story_scaler - 1))
...
static unsigned long obj_table_base = 0, obj_table_end = 0, obj_data_base = 0,
                     obj_data_end = 0;
...
#endif
{
    ...
    if (errflg || optind >= argc) {
        (void)fprintf(stderr,
                      "usage: %s [options...] story-file [story-file...]\n\n",
                      argv[0]);
        (void)fprintf(stderr, "TXD version 7/3 - disassemble Infocom story "
        ...
        (void)fprintf(stderr, "\t-u <file> Read user symbol table, implies -s "
                              "for Inform games\n");
        (void)fprintf(stderr,
...
#else
static void process_story(file_name) const char* file_name;
#endif
{
    ...
    if (header.version != V6 && header.version != V7) {
        ...
        decode.initial_pc =
            decode.pc + (unsigned long)header.start_pc * code_scaler;
    }

    tx_printf(
        "Resident data ends at %lx, program starts at %lx, file ends at %lx\n",
        (unsigned long)header.resident_size, (unsigned long)decode.initial_pc,
...
#    endif
    ...
    tx_printf("\nEnd of analysis pass, low address = %lx, high address = %lx\n",
              (unsigned long)decode.low_address,
              (unsigned long)decode.high_address);
    if (start_data_pc)
    ...
    if (option_labels) renumber_cref(routines_base);
...
#endif
{
    ...
    if (decode.first_pass) {
        if (decode.pc < decode.initial_pc) {
            ...
            for (pc = decode.pc, flag = 0; pc < decode.initial_pc && flag == 0;
                 pc += code_scaler) {
                for (vars = (char)read_data_byte(&pc); vars < 0 || vars > 15;
                ...
                for (i = 0, flag = 1; i < 3 && flag; i++) {
                    pcindex = 0;
                    decode.pc = ROUND_CODE(decode.pc);
                    if (decode_routine() != END_OF_ROUTINE || pcindex) flag = 0;
                }
                decode.pc = pc - 1;
            }
            if (flag && (unsigned int)header.version < V5) {
                ...
                vars = (char)read_data_byte(&pc);
                low_pc = decode.pc;
                for (pc = pc + (vars * 2) - 1, flag = 0;
                     pc >= low_pc && flag == 0; pc -= story_scaler) {
                    decode.pc = pc;
                    for (i = 0, flag = 1; i < 3 && flag; i++) {
                        pcindex = 0;
                        decode.pc = ROUND_CODE(decode.pc);
        ...
        do {
            ...
            while (decode.pc <= high_pc || decode.pc <= decode.initial_pc) {
                if (start_data_pc == decode.pc) decode.pc = end_data_pc;
                if (decode_routine() != END_OF_ROUTINE) {
                    if (start_data_pc == 0) start_data_pc = decode.pc;
                    flag = 1;
                    end_data_pc = 0;
                    pcindex = 0;
                    pc = ROUND_CODE(pc);
        ...
        tx_printf("]\n");
        for (decode.pc = decode.low_address; decode.pc <= decode.high_address;)
            (void)decode_routine();
        tx_printf("\n[End of code");
        if (option_labels == 0) tx_printf(" at %lx", (unsigned long)decode.pc);
        tx_printf("]\n");
    }
...
#endif
{
    ...
    if (decode.first_pass) {
        cref_item = NULL;
        if (option_labels) cref_item = current_routine;
        ...
        if (vars >= 0 && vars <= 15) {
            if (option_labels) add_routine(decode.pc - 1);
            locals_count = vars;
            if ((unsigned int)header.version < V5)
                for (; vars; vars--)
                    (void)read_data_word(&decode.pc);
            start_of_routine = decode.pc;
            if (decode_code() == END_OF_ROUTINE) return (END_OF_ROUTINE);
            if (option_labels) current_routine = cref_item;
            ...
            if (option_labels) {
                tx_printf("%soutine %c%04d, %d local",
                          (decode.pc - 1 == decode.initial_pc) ? "\nMain r"
                                                               : "\nR",
                          (option_inform) ? 'r' : 'R',
                          (int)lookup_routine(decode.pc - 1, 1), (int)vars);
            } else {
                tx_printf("%soutine %lx, %d local",
                          (decode.pc - 1 == decode.initial_pc) ? "\nMain r"
                                                               : "\nR",
                          (unsigned long)(decode.pc - 1), (int)vars);
            }
            if (vars != 1) tx_printf("s");
            if ((unsigned int)header.version < V5) {
                tx_printf(" (");
                tx_fix_margin(1);
                for (; vars; vars--) {
    ...
    do {
        if (decode.first_pass == 0) {
            if (option_labels) {
                ...
                if (label != 0)
                    tx_printf("%c%04d: ", (option_inform) ? 'l' : 'L',
                              (int)label);
                else
                    tx_printf("       ");
            } else
        ...
        else if (opcode.opcode < 0xb0)
            opcode.class = ONE_OPERAND;
        else if (opcode.opcode < 0xc0)
...
#define caseline(opc, text, par1, par2, par3, par4, extra, type)               \
    case opc:                                                                  \
        return (decode_operands(text, par1, par2, par3, par4, extra, type))
...
#endif
{
    ...
    case EXTENDED_OPERAND:
        code &= 0x3f;
        switch (code) {
            caseline(0x00, "SAVE", LOW_ADDR, NUMBER, LOW_ADDR, NIL, STORE,
                     PLAIN);
            caseline(0x01, "RESTORE", LOW_ADDR, NUMBER, LOW_ADDR, NIL, STORE,
                     PLAIN);
            ...
            caseline(0x05, "DRAW_PICTURE", NUMBER, NUMBER, NUMBER, NIL, NONE,
                     PLAIN);
            caseline(0x06, "PICTURE_DATA", NUMBER, LOW_ADDR, NIL, NIL, BRANCH,
                     PLAIN);
            caseline(0x07, "ERASE_PICTURE", NUMBER, NUMBER, NUMBER, NIL, NONE,
                     PLAIN);
            caseline(0x08, "SET_MARGINS", NUMBER, NUMBER, NUMBER, NIL, NONE,
                     PLAIN);
            caseline(0x09, "SAVE_UNDO", NIL, NIL, NIL, NIL, STORE, PLAIN);
            caseline(0x0A, "RESTORE_UNDO", NIL, NIL, NIL, NIL, STORE, PLAIN);

            caseline(0x10, "MOVE_WINDOW", NUMBER, NUMBER, NUMBER, NIL, NONE,
                     PLAIN);
            caseline(0x11, "WINDOW_SIZE", NUMBER, NUMBER, NUMBER, NIL, NONE,
                     PLAIN);
            caseline(0x12, "WINDOW_STYLE", NUMBER, NUMBER, NUMBER, NIL, NONE,
                     PLAIN);
            caseline(0x13, "GET_WIND_PROP", NUMBER, NUMBER, NIL, NIL, STORE,
                     PLAIN);
            caseline(0x14, "SCROLL_WINDOW", NUMBER, NUMBER, NIL, NIL, NONE,
                     PLAIN);
            caseline(0x15, "POP_STACK", NUMBER, LOW_ADDR, NIL, NIL, NONE,
                     PLAIN);
            caseline(0x16, "READ_MOUSE", LOW_ADDR, NIL, NIL, NIL, NONE, PLAIN);
            caseline(0x17, "MOUSE_WINDOW", NUMBER, NIL, NIL, NIL, NONE, PLAIN);
            caseline(0x18, "PUSH_STACK", NUMBER, LOW_ADDR, NIL, NIL, BRANCH,
                     PLAIN);
            caseline(0x19, "PUT_WIND_PROP", NUMBER, NUMBER, ANYTHING, NIL, NONE,
                     PLAIN);
            caseline(0x1A, "PRINT_FORM", LOW_ADDR, NIL, NIL, NIL, NONE, PLAIN);
            caseline(0x1B, "MAKE_MENU", NUMBER, LOW_ADDR, NIL, NIL, BRANCH,
                     PLAIN);
            caseline(0x1C, "PICTURE_TABLE", LOW_ADDR, NIL, NIL, NIL, NONE,
                     PLAIN);

        default:
            return (
                decode_operands("ILLEGAL", NIL, NIL, NIL, NIL, NONE, ILLEGAL));
        }
    ...
    case VARIABLE_OPERAND:
        code &= 0x3f;
        switch (code) {
            caseline(0x01, "JE", ANYTHING, ANYTHING, ANYTHING, ANYTHING, BRANCH,
                     PLAIN);
            ...
            caseline(0x0A, "TEST_ATTR", OBJECT, ATTRNUM, NIL, NIL, BRANCH,
                     PLAIN);
            caseline(0x0B, "SET_ATTR", OBJECT, ATTRNUM, NIL, NIL, NONE, PLAIN);
            caseline(0x0C, "CLEAR_ATTR", OBJECT, ATTRNUM, NIL, NIL, NONE,
                     PLAIN);
            ...
            caseline(0x12, "GET_PROP_ADDR", OBJECT, PROPNUM, NIL, NIL, STORE,
                     PLAIN);
            caseline(0x13, "GET_NEXT_PROP", OBJECT, PROPNUM, NIL, NIL, STORE,
                     PLAIN);
            ...
            caseline(0x21, "STOREW", LOW_ADDR, NUMBER, ANYTHING, NIL, NONE,
                     PLAIN);
            caseline(0x22, "STOREB", LOW_ADDR, NUMBER, ANYTHING, NIL, NONE,
                     PLAIN);
            caseline(0x23, "PUT_PROP", OBJECT, NUMBER, ANYTHING, NIL, NONE,
                     PLAIN);
            ...
            caseline(0x33, "OUTPUT_STREAM", PATTR, LOW_ADDR, NUMBER, NIL, NONE,
                     PLAIN);
            caseline(0x34, "INPUT_STREAM", NUMBER, NIL, NIL, NIL, NONE, PLAIN);
            caseline(0x35, "SOUND_EFFECT", NUMBER, NUMBER, NUMBER, ROUTINE,
                     NONE, PLAIN);
        ...
        default:
            ...
            case V3:
                switch (code) {
                    caseline(0x20, "CALL", ROUTINE, ANYTHING, ANYTHING,
                             ANYTHING, STORE, CALL);

                    caseline(0x24, "READ", LOW_ADDR, LOW_ADDR, NIL, NIL, NONE,
                             PLAIN);
            ...
            case V4:
                switch (code) {
                    caseline(0x19, "CALL_2S", ROUTINE, ANYTHING, NIL, NIL,
                             STORE, CALL);

                    caseline(0x20, "CALL_VS", ROUTINE, ANYTHING, ANYTHING,
                             ANYTHING, STORE, CALL);

                    caseline(0x24, "READ", LOW_ADDR, LOW_ADDR, NUMBER, ROUTINE,
                             NONE, PLAIN);

                    caseline(0x29, "PULL", VAR, NIL, NIL, NIL, NONE, PLAIN);

                    caseline(0x2C, "CALL_VS2", ROUTINE, ANYTHING, ANYTHING,
                             ANYTHING, STORE, CALL);
                    caseline(0x2D, "ERASE_WINDOW", NUMBER, NIL, NIL, NIL, NONE,
                             PLAIN);
                    caseline(0x2E, "ERASE_LINE", NUMBER, NIL, NIL, NIL, NONE,
                             PLAIN);
                    caseline(0x2F, "SET_CURSOR", NUMBER, NUMBER, NIL, NIL, NONE,
                             PLAIN);

                    caseline(0x31, "SET_TEXT_STYLE", VATTR, NIL, NIL, NIL, NONE,
                             PLAIN);
                    caseline(0x32, "BUFFER_MODE", NUMBER, NIL, NIL, NIL, NONE,
                             PLAIN);

                    caseline(0x36, "READ_CHAR", NUMBER, NUMBER, ROUTINE, NIL,
                             STORE, PLAIN);
                    caseline(0x37, "SCAN_TABLE", ANYTHING, LOW_ADDR, NUMBER,
                             NUMBER, BOTH, PLAIN);
                }
            case V5:
            case V7:
            case V8:
                switch (code) {
                    caseline(0x19, "CALL_2S", ROUTINE, ANYTHING, NIL, NIL,
                             STORE, CALL);
                    caseline(0x1A, "CALL_2N", ROUTINE, ANYTHING, NIL, NIL, NONE,
                             CALL);
                    caseline(0x1B, "SET_COLOUR", NUMBER, NUMBER, NIL, NIL, NONE,
                             PLAIN);
                    ...
                    caseline(0x39, "CALL_VN", ROUTINE, ANYTHING, ANYTHING,
                             ANYTHING, NONE, CALL);
                    caseline(0x3A, "CALL_VN2", ROUTINE, ANYTHING, ANYTHING,
                             ANYTHING, NONE, CALL);
                    caseline(0x3B, "TOKENISE", LOW_ADDR, LOW_ADDR, LOW_ADDR,
                             NUMBER, NONE, PLAIN);
                    caseline(0x3C, "ENCODE_TEXT", LOW_ADDR, NUMBER, NUMBER,
                             LOW_ADDR, NONE, PLAIN);
                    caseline(0x3D, "COPY_TABLE", LOW_ADDR, LOW_ADDR, NUMBER,
                             NIL, NONE, PLAIN);
                    caseline(0x3E, "PRINT_TABLE", LOW_ADDR, NUMBER, NUMBER,
                             NUMBER, NONE, PLAIN);
                    caseline(0x3F, "CHECK_ARG_COUNT", NUMBER, NIL, NIL, NIL,
                             BRANCH, PLAIN);
                }
            case V6:
                switch (code) {
                    ...
                    caseline(0x1B, "SET_COLOUR", NUMBER, NUMBER, NIL, NIL, NONE,
                             PLAIN);
                    caseline(0x1C, "THROW", ANYTHING, NUMBER, NIL, NIL, NONE,
                             PLAIN);
                    ...
                    caseline(0x29, "PULL", LOW_ADDR, NIL, NIL, NIL, STORE,
                             PLAIN);
                    ...
                    caseline(0x2E, "ERASE_LINE", NUMBER, NIL, NIL, NIL, NONE,
                             PLAIN);
                    caseline(0x2F, "SET_CURSOR", NUMBER, NUMBER, NUMBER, NIL,
                             NONE, PLAIN);
                    caseline(0x30, "GET_CURSOR", LOW_ADDR, NIL, NIL, NIL, NONE,
                             PLAIN);
                    caseline(0x31, "SET_TEXT_STYLE", VATTR, NIL, NIL, NIL, NONE,
                             PLAIN);
                    caseline(0x32, "BUFFER_MODE", NUMBER, NIL, NIL, NIL, NONE,
                    ...
                    caseline(0x3C, "ENCODE_TEXT", LOW_ADDR, NUMBER, NUMBER,
                             LOW_ADDR, NONE, PLAIN);
                    caseline(0x3D, "COPY_TABLE", LOW_ADDR, NUMBER, LOW_ADDR,
                             NIL, NONE, PLAIN);
                    caseline(0x3E, "PRINT_TABLE", LOW_ADDR, NUMBER, NUMBER,
                             NUMBER, NONE, PLAIN);
                    caseline(0x3F, "CHECK_ARG_COUNT", NUMBER, NIL, NIL, NIL,
    ...
    case ONE_OPERAND:
        ...
        switch (code) {
            ...
            caseline(0x04, "GET_PROP_LEN", LOW_ADDR, NIL, NIL, NIL, STORE,
                     PLAIN);
        ...
        default:
            ...
            case V4:
                switch (code) {
                    caseline(0x08, "CALL_1S", ROUTINE, NIL, NIL, NIL, STORE,
                             CALL);
            ...
            case V8:
                switch (code) {
                    ...
                    caseline(0x0F, "CALL_1N", ROUTINE, NIL, NIL, NIL, NONE,
                             CALL);
                }
            }
            return (
        ...
        default:
            ...
            case V3:
                switch (code) {
                    ...
                    caseline(0x06, "RESTORE", NIL, NIL, NIL, NIL, BRANCH,
                             PLAIN);

                    caseline(0x09, "POP", NIL, NIL, NIL, NIL, NONE, PLAIN);

                    caseline(0x0C, "SHOW_STATUS", NIL, NIL, NIL, NIL, NONE,
                             PLAIN);
                }
            case V4:
                switch (code) {
...
#ifdef __STDC__
static int decode_operands(const char* opcode_name, int par1, int par2,
                           int par3, int par4, int extra, int type)
#else
static int decode_operands(opcode_name, par1, par2, par3, par4, extra,
                           type) const char* opcode_name;
int par1;
int par2;
int par3;
int par4;
...
#endif
{
    ...
    if (opcode.type == ILLEGAL) return (BAD_OPCODE);

    if (decode.first_pass) {
        status = decode_parameters(&opers);
        if (status) return (BAD_OPCODE);
        status = decode_extra();
    } else {
        if (option_dump)
            dump_opcode(decode.pc, opcode.opcode, opcode.class, opcode.par,
                        opcode.extra);
        if (option_inform) {
        ...
        if (opers > 0 && opcode.extra != NONE) tx_printf(" ");
        status = decode_extra();
        tx_printf("\n");
    }
    if (decode.pc > decode.high_pc) decode.high_pc = decode.pc;
...
#endif
{
    ...
    *opers = 0;
    ...
    case TWO_OPERAND:
        status =
            decode_parameter((opcode.opcode & 0x40) ? VARIABLE : BYTE_IMMED, 0);
        if (status) return (status);
        if (decode.first_pass == 0) {
        ...
        status =
            decode_parameter((opcode.opcode & 0x20) ? VARIABLE : BYTE_IMMED, 1);
        if (status) return (status);
        *opers = 2;
        if (!option_inform && decode.first_pass == 0 && opcode.type == CALL &&
            *opers > 1)
            tx_printf(")");
        break;
    ...
    case VARIABLE_OPERAND:
    case EXTENDED_OPERAND:
        if ((opcode.opcode & 0x3f) == 0x2c || (opcode.opcode & 0x3f) == 0x3a) {
        ...
        for (addr_mode = 0, *opers = 0; (addr_mode != NO_OPERAND) && maxopers;
             maxopers--) {
...
#endif
{
    ...
    case ANYTHING:
        if (decode.first_pass == 0) {
            ...
            if (dictionary) {
                if (string) tx_printf(" %s ", (option_inform) ? "or" : "OR");
                tx_printf("\"");
        ...
        if (decode.first_pass == 0) {
            if (option_labels)
                if (value != 0) {
                    ...
                    if (routine != 0)
                        tx_printf("%c%04d", (option_inform) ? 'r' : 'R',
                                  routine);
                    else {
                        (void)fprintf(
                            stderr,
            ...
            if (addr < decode.low_address && addr >= code_base) {
                vars = read_data_byte(&addr);
                if (vars >= 0 && vars <= 15) decode.low_address = addr - 1;
            }
            if (addr > decode.high_address && addr < file_size) {
                vars = read_data_byte(&addr);
                if (vars >= 0 && vars <= 15) decode.high_address = addr - 1;
            }
        }
        break;
    ...
    case LABEL:
        ...
        if (decode.first_pass && addr < decode.low_address) return (1);
        if (option_labels) {
            if (decode.first_pass)
                add_label(addr);
            else
            ...
            if (decode.first_pass == 0) tx_printf("%lx", addr);
        }
        if (addr > decode.high_pc) decode.high_pc = addr;
        break;
    ...
    case PATTR:
        if (decode.first_pass == 0) {
            if ((int)value == 1)
                tx_printf("%s",
                          (option_inform) ? "output_enable" : "OUTPUT_ENABLE");
            else if ((int)value == 2)
                tx_printf("%s", (option_inform) ? "scripting_enable"
                                                : "SCRIPTING_ENABLE");
            else if ((int)value == 3)
                tx_printf("%s", (option_inform) ? "redirect_enable"
                                                : "REDIRECT_ENABLE");
            else if ((int)value == 4)
                tx_printf("%s",
                          (option_inform) ? "record_enable" : "RECORD_ENABLE");
            else if ((int)value == -1)
                tx_printf("%s", (option_inform) ? "output_disable"
                                                : "OUTPUT_DISABLE");
            else if ((int)value == -2)
                tx_printf("%s", (option_inform) ? "scripting_disable"
                                                : "SCRIPTING_DISABLE");
            else if ((int)value == -3)
                tx_printf("%s", (option_inform) ? "redirect_disable"
                                                : "REDIRECT_DISABLE");
            else if ((int)value == -4)
                tx_printf("%s", (option_inform) ? "record_disable"
                                                : "RECORD_DISABLE");
            else
                print_integer(value, addr_mode == BYTE_IMMED);
        }
...
#endif
{
    ...
    if (opcode.extra == STORE || opcode.extra == BOTH) {
        ...
        if (decode.first_pass == 0) {
            if (!option_inform || (option_inform >= 6)) tx_printf("-> ");
            if (addr == 0)
                tx_printf("%s", (option_inform) ? "sp" : "-(SP)");
            else
                print_variable(addr);
...
#endif
            if (opcode.extra == BOTH) tx_printf(" ");
        } else {
    ...
    if (opcode.extra == BRANCH || opcode.extra == BOTH) {
        ...
        if (decode.first_pass == 0) {
            if ((addr > 1) && !(firstbyte & 0x40) && (option_inform >= 6) &&
                (option_labels)) {
        ...
        if (addr == 0) {
            ...
            if (option_labels) {
                if (decode.first_pass)
                    ...
                    tx_printf("%s%04d", (option_inform) ? "l" : " L",
                              (int)lookup_label(addr, 1));
            } else {
                if (decode.first_pass == 0)
                    tx_printf("%s%lx", (option_inform) ? "" : " ",
                              (unsigned long)addr);
            }
            if (addr > decode.high_pc) decode.high_pc = addr;
        }
    ...
    if (opcode.type == RETURN)
        if (decode.pc > decode.high_pc) return (END_OF_ROUTINE);
...
#else
static void decode_strings(pc) unsigned long pc;
#endif
{
    int count = 1;
    ...
    pc = ROUND_DATA(pc);
    tx_printf("\n[Start of text");
    if (option_labels == 0) tx_printf(" at %lx", (unsigned long)pc);
    tx_printf("]\n\n");
    while (pc < file_size) {
        if (option_labels)
...
#else
static void scan_strings(pc) unsigned long pc;
#endif
{
    ...
    while (pc < file_size) {
        ...
        pc = ROUND_CODE(pc);
        if ((unsigned int)data & 0x8000) old_pc = pc;
    }
    file_size = old_pc;
...
#endif
{
    ...
    if (addr <= decode.high_address || addr >= file_size) return (0);

    for (cref_item = strings_base; cref_item != NULL;
         cref_item = cref_item->next)
        if (cref_item->address == addr) return (cref_item->number);
...
#else
static void lookup_verb(addr) unsigned long addr;
#endif
{
    ...
    for (i = 0; i < action_count; i++) {
        routine = (unsigned long)read_data_word(&address) * code_scaler +
                  (unsigned long)story_scaler * header.routines_offset;
        if (routine == addr) {
            if (first) {
            ...
            show_syntax_of_action(i, verb_table_base, verb_count, parser_type,
    ...
    if (parser_type >= inform_gv2) {
        if (is_gv2_parsing_routine(addr, verb_table_base, verb_count)) {
            tx_printf("    Parsing routine for:\n");
            tx_printf("        ");
            tx_fix_margin(1);
            first = 0;
            show_syntax_of_parsing_routine(addr, verb_table_base, verb_count,
                                           parser_type, prep_type,
        ...
        for (i = 0; i < parse_count; i++) {
            ...
            if (routine == addr) {
                ...
                show_syntax_of_parsing_routine(
                    i, verb_table_base, verb_count, parser_type, prep_type,
                    prep_table_base, attr_names_base);
            }
        }
    } else {
        for (i = 0; i < action_count; i++) {
            ...
            if (routine == addr) {
                ...
                show_syntax_of_action(i, verb_table_base, verb_count,
                                      parser_type, prep_type, prep_table_base,
                                      attr_names_base);
            }
        }
    }
...
#endif
{
    ...
    if (word_address < dict_start || word_address > dict_end) return (0);

    if ((word_address - dict_start) % word_size == 0) return (1);
...
#else
static void add_label(addr) unsigned long addr;
#endif
{
    cref_item_t *cref_item, **prev_item, *next_item;

    if (current_routine == NULL) return;
...
#else
static void add_routine(addr) unsigned long addr;
#endif
{
    cref_item_t *cref_item, **prev_item, *next_item;
...
#endif
{
    ...
    if (flag) current_routine = cref_item;
...
#else
static void renumber_cref(cref_item) cref_item_t* cref_item;
#endif
{
    int number = 1;

    while (cref_item != NULL) {
        if (start_data_pc == 0 || cref_item->address < start_data_pc ||
...
#else
static void free_cref(cref_item) cref_item_t* cref_item;
#endif
{
    cref_item_t* t;
...
#endif
{
    ...
    if ((unsigned int)header.version < V4)
        address += ((P3_MAX_PROPERTIES - 1) * 2) + ((obj - 1) * O3_SIZE) +
                   O3_PROPERTY_OFFSET;
    else
        address += ((P4_MAX_PROPERTIES - 1) * 2) + ((obj - 1) * O4_SIZE) +
                   O4_PROPERTY_OFFSET;
...
#else
static void print_text(addr) unsigned long* addr;
#endif
{
...
#else
static void print_integer(value, flag) unsigned int value;
int flag;
#endif
{
...
#else
static void dump_data(start_addr, end_addr) unsigned long start_addr;
...
#ifdef __STDC__
static void dump_opcode(unsigned long addr, int op, int class, int* par,
                        int extra)
#else
static void dump_opcode(addr, op, class, par, extra) unsigned long addr;
int op;
int class;
int* par;
int extra;
#endif
{
    ...
    if (class == TWO_OPERAND) {
        dump_operand(&addr, (op & 0x40) ? VARIABLE : BYTE_IMMED, 0, par,
                     &count);
        dump_operand(&addr, (op & 0x20) ? VARIABLE : BYTE_IMMED, 1, par,
                     &count);
    }
...
#ifdef __STDC__
static void dump_operand(unsigned long* addr, int addr_mode, int opers,
                         int* par, int* count)
#else
static void dump_operand(addr, addr_mode, opers, par,
...
#endif
{
    ...
    if (opers < 4 && par[opers] == VAR) addr_mode = VARIABLE;
...
#else
static void print_variable(varnum) int varnum;
#endif
{
    if (varnum < 16) {
        if (option_symbols &&
