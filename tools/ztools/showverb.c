 ...
 *	  pointers. The format of the parse data varies between games.
 *Basically, each entry has a count (1 byte) of parse structures corresponding
 *to different sentence structures. For example, the verb 'take' in the
 *	  dictionary may have two forms; 'take object' or 'take object with
 *object'. Each form has an index (1 byte) into the pre-action and action
 *routine tables.
 *
 ...
 *
 *	  Next come the pre-action routine tables. There is an entry (2 bytes)
 *for each verb form index. The entries are packed addresses of the Z-code
 ...
 *
 *	  verb [+ prep] 'look [at]' verb [+ prep] + object
 *'look [at] book' verb [+ prep] + object [+ prep] + object	 'look [at] book
 *[with] glasses'
 *
 *	  The verb and prepositions can only be a single word each. The object
 *	  could be a multiple words 'green book' or a list depending on the
 *object data.
 *
 * Notes:
 *
 ...
 *
 * Bytes 6-7: (Two object entries only) Same as bytes 2-3, for second
 *preposition Bytes 8-9: (Two object entries only) Same as bytes 4-5, for second
 *object
 *
 *
 * Note also that the dictionary flags have moved from the first to the last
 *byte of each dictionary entry, and that while Zork Zero has only three bytes
 *of extra dictionary data (as with V1-5), Shogun and Arthur have four. Also, I
 *believe there is more grammar data than I've listed here, though how much is
 *for the parser proper and how much for the helper I don't know -- MTR
 *
...
void configure_parse_tables(
    ...
    unsigned int* prep_type, unsigned long* verb_table_base,
    ...
    unsigned long* prep_table_end)
{
    ...
    *prep_type = 0;
    ...
    if (header.serial[0] >= '0' && header.serial[0] <= '9' &&
        ...
        *parser_type = inform5_grammar;
        ...
        if (header.name[4] >= '6') *parser_type = inform_gv1;
    }

    if ((*parser_type < inform5_grammar) &&
        (unsigned int)header.version == V6) {
        ...
        *preact_table_base = (unsigned short)read_data_word(&address);
        ...
        for (word_address = first_word; word_address <= last_word;
            ...
            if ((val & 1) && !(val & 0x80) && (parse_index != 0) &&
                (parse_index < *action_table_base)) { /* dictionary verb */
                if (vbase > parse_index) vbase = parse_index;
                if (vend <= parse_index) vend = parse_index + 8;
        ...
        *verb_data_base = area2base;
        /* there is no preposition table, but *prep_table_base bounds the verb
         * data area */
    ...
    *verb_data_base = first_entry;
    ...
    /* Distinguishing between GV1 and GV2 isn't trivial.
       ...
       If they aren't found, I assume GV1.  I believe it is actually possible
       for a legal (if somewhat nonsensical) GV1 table to be the same as a legal
       GV2 table, but I haven't actually constructed such a weird table.  In
       practice, the ENDIT (15) byte of the GV2 table will probably cause an
       illegal token if the table is interpreted as GV1 -- MTR.
    */
    ...
    if (*parser_type == inform_gv1) {
        ...
        if (((second_entry - first_entry) % 3) == 1) {
            ...
            if ((entry_count * 8 + 1) == (second_entry - first_entry)) {
                ...
                for (i = 0; i < entry_count && (*parser_type == inform_gv1);
                     i++) {
                    ...
                    for (j = 1; j < 7; j++) {
                        ...
                        if ((val >= 9) || (val <= 15) || (val >= 112) ||
                            (val <= 127)) {
                            *parser_type = inform_gv2;
                            break;
                        }
                    }
                    read_data_byte(&first_entry); /* action number.  This can be
                                                     anything */
                }
            } else {
                *parser_type = inform_gv2;
    ...
    *parse_count = 0;
        ...
        while (entry_count--) {
            if (*parser_type == infocom_fixed) {
                ...
                for (j = 0; j < 6; j++) {
                    ...
                    if (val < 16 || val >= 112) continue;
                    parse_index = (val - 16) % 32;
                    if (parse_index > *parse_count) *parse_count = parse_index;
                }
                action_index = (unsigned int)read_data_byte(&verb_entry);
            } else {
                /* GV2 */
                action_index =
                    (unsigned int)(read_data_word(&verb_entry) & 0x3FF);
                val = read_data_byte(&verb_entry);
                while (val != 15) {
            ...
            if (action_index > *action_count) *action_count = action_index;
        }
    }
    (*action_count)++;
    ...
    while ((unsigned int)read_data_byte(&verb_entry) ==
           0) /* Skip padding, if any */
        ;
    ...
    *preact_table_base = *action_table_base + (*action_count * sizeof(zword_t));
    ...
    if (*parser_type >= inform_gv2) {
        ...
        *prep_table_end = *preact_table_base;
        ...
        if (*parser_type < inform_gv1)
            *prep_table_base =
                *preact_table_base + (*action_count * sizeof(zword_t));
        else
            *prep_table_base =
...
void show_verbs(int symbolic)
{
    ...
    show_action_tables(verb_table_base, verb_count, action_count, parse_count,
                       parser_type, prep_type, action_table_base,
                       preact_table_base, prep_table_base, attr_names_base,
                       action_names_base);

    /* Display prepositions */
    if ((parser_type <= inform_gv2) &&
        (parser_type != infocom6_grammar)) /* no preposition table in GV2 */
                                  ...
                                  unsigned long attr_names_base)
{
    ...
    for (i = 0; (unsigned int)i < verb_count; i++) {
        ...
        if (parser_type == infocom6_grammar) {
            ...
            verb_address =
                (unsigned int)address; /* cast is guaranteed to work provided
            ...
            if (verb_entry != 0xFFFF)
                show_verb_grammar(parse_entry, verb_address, (int)parser_type,
                                  0, 0, 0L, 0L);
            ...
            if (do_address) {
                ...
                while (entry_count--) {
                    ...
                    for (j = 0; j < verb_size; j++) {
                        ...
                        tx_printf("%04x", (unsigned int)parse_data);
                        if (j < (verb_size - 1)) tx_printf(" ");
                    }
                    tx_printf("] ");
                    show_verb_grammar(parse_entry, verb_address,
                                      (int)parser_type, 1, 0, 0L, 0L);
                    tx_printf("\n");
                }
            }
            if (doio_address) {
            ...
            while (entry_count--) {
                ...
                if (parser_type < inform_gv2) {
                    ...
                    /* GV2 variable entry format
                       <flags and action high> <action low> n*(<token type>
                       <token data 1> <token data 2>) <ENDIT>*/
                    for (j = 0;
                         (j == 0) || (j % 3 != 0) || (parse_data != ENDIT);
                         j++) {
                        if (j != 0) tx_printf(" ");
                        parse_data = (unsigned int)read_data_byte(&verb_entry);
                        tx_printf("%02x", (unsigned int)parse_data);
                ...
                show_verb_grammar(parse_entry, VERB_NUM(i, parser_type),
                                  (int)parser_type, 0, (int)prep_type,
                                  prep_table_base, attr_names_base);
                tx_printf("\n");
            }
        }
        ...
        if (parser_type == infocom6_grammar) {
            ...
            if (doio_address) {
                ...
                while (entry_count--) {
                    ...
                    verb_entry += 8; /* skip preposition and direct object and
            ...
            while (entry_count--) {
                parse_entry = verb_entry;
                if (parser_type >=
                    inform_gv2) { /* GV2, variable length with terminator */
                    ...
                    while (val != ENDIT) {
                        ...
                        val = read_data_byte(&verb_entry);
                           ...
                           infocom_variable) { /* Index is in last (8th) byte */
                    verb_entry += 7;
                    action_index = (unsigned int)read_data_byte(&verb_entry);
                           ...
                           unsigned int verb_count)
{
    ...
    int i, found;
    unsigned long parsing_routine_packed =
        (parsing_routine -
    ...
    for (i = 0; !found && (unsigned int)i < verb_count; i++) {
        ...
        while (!found && entry_count--) {
            ...
            while (val != ENDIT) {
                ...
                if (((val & 0xC0) == 0x80) &&
                    (token_data == parsing_routine_packed))
                    found = 1;
                val = (unsigned int)read_data_byte(&verb_entry);
            }
        }
 ...
 *
 * Display the syntax entries for a given parsing routine number or address.
 * Used by txd as well as by show_action_tables. For Inform 5 and GV1, the input
 * should be the parsing routine number.	For GV2, it should be the
 * parsing routine address.
 *
 */
        ...
        while (entry_count--) {
            ...
            if (parser_type < inform_gv2) {
                ...
                while (object_count) {
                    ...
                    if (val < 0xb0) {
                        object_count--;
                        if (val >= 0x10 && val < 0x70 &&
                            ((val - 0x10) & 0x1f) ==
                                (unsigned int)parsing_routine)
                            found = 1;
                    }
                }
                ...
                while (val != ENDIT) {
                    ...
                    if (((val & 0xC0) == 0x80) &&
                        (token_data ==
                         parsing_routine_packed)) /* V7/V6 issue here */
                        found = 1;
                    val = (unsigned int)read_data_byte(&verb_entry);
                }
            }
            if (found) {
                show_verb_grammar(parse_entry, VERB_NUM(i, parser_type),
                                  (int)parser_type, (int)prep_type, 0,
                                  prep_table_base, attr_names_base);
                tx_printf("\n");
            }
        }
...
show_action_tables(unsigned long verb_table_base, unsigned int verb_count,
                   ...
                   unsigned int parser_type, unsigned int prep_type,
                   ...
                   unsigned long action_names_base)
{
    ...
    tx_printf("action# ");
    if (parser_type <= infocom6_grammar) tx_printf("pre-action-routine ");
    ...
    for (action = 0; (unsigned int)action < action_count; action++) {
        ...
        if (parser_type <= infocom6_grammar) {
            routine_address =
                (unsigned long)read_data_word(&preacts_address) * code_scaler;
            if (routine_address)
                routine_address +=
                    (unsigned long)story_scaler * header.routines_offset;
        ...
        routine_address =
            (unsigned long)read_data_word(&actions_address) * code_scaler;
        if (routine_address)
            routine_address +=
                (unsigned long)story_scaler * header.routines_offset;
        tx_printf("%5lx ", routine_address);
         ...
         */
        show_syntax_of_action(action, verb_table_base, verb_count, parser_type,
    ...
    if ((parser_type >= inform5_grammar) && (parser_type < inform_gv2)) {
        ...
        for (action = 0; (unsigned int)action < parse_count; action++) {
            ...
            tx_printf("%3d. ", (int)action);
            tx_printf("%5lx ",
                      (unsigned long)read_data_word(&preacts_address) *
                              code_scaler +
                          (unsigned long)story_scaler * header.routines_offset);
            tx_printf(" ");
            tx_fix_margin(1);
            /*
             * Now scan down the parse table looking for all verb/sentence
             * formats that this parsing routine applies to.
             */

            show_syntax_of_parsing_routine(
                (unsigned long)action, verb_table_base, verb_count, parser_type,
                prep_type, prep_table_base, attr_names_base);
            tx_fix_margin(0);
        }
    }
                       ...
                       unsigned int type, unsigned int parser_type)
{
    ...
    if (word_address == 0) {
        tx_printf(" no-");
        if ((type == VERB) || (type == VERB_V6)) tx_printf("verb");
        if (type == PREP) tx_printf("preposition");
    }
    ...
    for (flag = 0; word_address; flag++) {
        ...
        if (type == PREP && flag == 0) word_address = 0;
        if (type != PREP || parser_type <= infocom_variable) {
            ...
            if (type == PREP && word_address == prep_address)
                word_address =
                    lookup_word(word_address, indx, type, parser_type);
        }
    }
    if (flag) tx_fix_margin(0);
    ...
    if (parser_type == infocom6_grammar) {
        ...
        if (v6_number_objects > 0) {
            ...
            while (v6_number_objects--) {
                ...
                tx_printf("OBJ");
#endif
                if (v6_number_objects) tx_printf(" ");
            }
        }
        tx_printf("\"");
        ...
        while (val != ENDIT) {
            if (((val & 0x30) == 0x10) ||
                ((val & 0x30) ==
                 0x30)) /* 2nd ... nth byte of alternative list */
                tx_printf(" /");
            tx_printf(" ");
            token_type = val & 0xF;
            token_data = read_data_word(&address);
        ...
        tx_printf("\"");
        if (action & 0x0400) tx_printf(" REVERSE");
        ...
        if (parser_type == infocom_fixed) {
            ...
            /* 1st preposition in bottom 6 bits of first byte. Fill in top two
             * bits */
            ...
            if (objs > 0) {
                ...
                if (objs > 1) {
                    ...
                    /* 2nd preposition in bottom 6 bits of byte. Fill in top two
                     * bits */
        ...
        if (objs > 2) {
            ...
            for (i = 0; i < 2; i++) {
                ...
                if (objs > (unsigned int)i) tx_printf(" OBJ");
            }
                                 ...
                                 unsigned int mask, unsigned int parser_type)
{
    ...
    /* Correct Inform verb mask -- Inform sets both 0x40 and 0x01, but only 0x01
     * is documented */
    if ((mask == VERB) && (parser_type >= inform5_grammar)) mask = VERB_INFORM;

    /* Scan down the dictionary from entry looking for a match */

    for (word_address = entry; word_address <= last_word;
         word_address += word_size) {
        ...
        if (parser_type != infocom6_grammar) {
            address =
                word_address + (((unsigned int)header.version < V4) ? 4 : 6);
        ...
        if (flags & mask) {
            ...
            if (data == number) return (word_address);
        }
    }
