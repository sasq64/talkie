...
#endif
{
    ...
    if (argc < 2) {
        ...
        (void)fprintf(
            stderr,
            "CHECK version 7/2 - verify Infocom story files. By Mark Howell\n");
    ...
    if (h_file_size == 0) {
        (void)fprintf(
            stdout, "Story has no file size in the header\nEnter file size:\n");
    ...
    while (h_file_size) {
        ...
        h_file_size -= size;
        size =
            (h_file_size >= PAGE_SIZE) ? PAGE_SIZE : (unsigned int)h_file_size;
    }
    ...
    if (checksum == h_checksum)
        ...
        (void)printf(
            "Checksum bad! actual checksum = %04x, required checksum = %04x\n",
