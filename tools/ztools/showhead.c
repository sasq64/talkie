...
static const char* interpreter_flags1[] = {
    "Byte swapped data",       "Display time",
    "Unknown (0x04)",          "Tandy",
    "No status line",          "Windows available",
    "Proportional fonts used", "Unknown (0x80)"};

static const char* interpreter_flags2[] = {
    "Colours",          "Pictures",       "Bold font",      "Emphasis",
    "Fixed space font", "Unknown (0x20)", "Unknown (0x40)", "Timed input"};

static const char* game_flags1[] = {
    "Scripting",        "Use fixed font",   "Unknown (0x0004)",
    "Unknown (0x0008)", "Supports sound",   "Unknown (0x0010)",
    "Unknown (0x0020)", "Unknown (0x0040)", "Unknown (0x0080)",
    "Unknown (0x0200)", "Unknown (0x0400)", "Unknown (0x0800)",
    "Unknown (0x1000)", "Unknown (0x2000)", "Unknown (0x4000)",
    "Unknown (0x8000)"};

static const char* game_flags2[] = {
    "Scripting",         "Use fixed font",   "Screen refresh required",
    "Supports graphics", "Supports undo",    "Supports mouse",
    "Supports colour",   "Supports sound",   "Supports menus",
    "Unknown (0x0200)",  "Printer error",    "Unknown (0x0800)",
    "Unknown (0x1000)",  "Unknown (0x2000)", "Unknown (0x4000)",
    "Unknown (0x8000)"};
...
#endif
{
    ...
    if (header.serial[0] >= '0' && header.serial[0] <= '9' &&
        ...
        if (header.name[4] >= '6') inform = header.name[4] - '0';
    }
    ...
    for (i = 0; i < 8; i++) {
        if ((unsigned int)header.config & (1 << i)) {
            tx_printf("%s%s", (list++) ? ", " : "",
                      ((unsigned int)header.version < V4)
                          ? interpreter_flags1[i]
                          : interpreter_flags2[i]);
        } else {
            if ((unsigned int)header.version < V4 && i == 1)
                tx_printf("%sDisplay score/moves", (list++) ? ", " : "");
        }
    }
    if (list == 0) tx_printf("None");
    tx_printf("\n");
    tx_fix_margin(0);
    ...
    if ((unsigned int)header.version != V6)
        ...
        tx_printf(
            "Main routine address:     %05lx\n",
            (unsigned long)(((unsigned long)header.start_pc * code_scaler) +
                            ((unsigned long)header.routines_offset *
                             story_scaler)));
    ...
    for (i = 0; i < 16; i++) {
        if ((unsigned int)header.flags & (1 << i)) {
            tx_printf("%s%s", (list++) ? ", " : "",
                      ((unsigned int)header.version < V4) ? game_flags1[i]
                                                          : game_flags2[i]);
        }
    }
    if (list == 0) tx_printf("None");
    ...
    case 5:
        ...
        break;
    case 6:
        tx_printf("IBM/MS-DOS");
        break;
    case 7:
        tx_printf("Commodore 128");
        break;
    case 8:
        tx_printf("C64");
        break;
    ...
    case 10:
        ...
        break;
    case 11:
        tx_printf("Tandy Color Computer");
        break;
    default:
        tx_printf("Unknown");
    }
    ...
    if ((unsigned int)header.alphabet) {
        ...
        for (i = 0; i < 3; i++) {
            tx_printf("\"");
            for (j = 0; j < 26; j++)
                tx_printf("%c", (char)get_byte((unsigned int)header.alphabet +
                                               (i * 26) + j));
            tx_printf("\"\n");
        }
        tx_fix_margin(0);
    ...
    if ((unsigned int)header.name[0] || (unsigned int)header.name[1] ||
        (unsigned int)header.name[2] || (unsigned int)header.name[3] ||
        (unsigned int)header.name[4] || (unsigned int)header.name[5] ||
        (unsigned int)header.name[6] || (unsigned int)header.name[7]) {
        tx_printf("Name:                     \"");
