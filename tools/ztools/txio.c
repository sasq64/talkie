    ...
    do {
        ...
        for (i = 10; i >= 0; i -= 5) {
            ...
            if (synonym_flag) {
                ...
                addr =
                    (unsigned long)get_word((unsigned int)header.abbreviations +
                                            synonym + (code * 2)) *
                    2;
                ...
                /*
                 * If this is character 0 in the punctuation set then the next
                 * two codes make an ASCII character.
                 */

                if (shift_state == 2 && code == 0) ascii_flag = 1;
                ...
                if (code == 0) {
                    ...
                    if ((unsigned int)header.version < V3) {
                        ...
                        if (code == 1) {
                             ...
                             *
                             * Shift keys 2 & 3 only shift the next character
                             * and can be used regardless of the state of the
                             * shift lock. Shift keys 4 & 5 lock the shift until
                             * reset.
                             *
                             * The following code implements the the shift code
                             * state transitions:
                             *
                             *               +-------------+-------------+-------------+-------------+
                             *               |       Shift   State       | Lock
                             ...
                             * uppercase   | punctuation | | uppercase   |
                             * punctuation | lowercase   | punctuation |
                             * lowercase   | | punctuation | lowercase   |
                        ...
                        if (code < 4) {
                             ...
                             * either uppercase (4) or punctuation (5). The
                             * shift state automatically gets reset back to
                             * lowercase for V3+ games after the next character
                             * is output.
                             *
                             */
