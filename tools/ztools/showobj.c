 ...
 * Property zero always terminates the list, but is not referenced by the
 * Z-code. Instead, property zero is used to terminate the scan down the
 * property list, if a property is not defined. This behaviour is required when
 * loading a default property list item, or to catch setting undefined property
 * values.
 *
 * The size information is ignored for property 0, which is actually just
 * specified as a byte containing 0x00.
 *
 * Key:
 *
 ...
 * +-----------+-----------+----+ +-----------+---------+------+ +------+
 * +------+ | size byte | word data | id | | size byte | ignored | size | | data
 * | ... | data | max=63, min=0
 * +-----------+-----------+----+ +-----------+---------+------+ +------+
 * +------+ 7           6 5  0             7         6 5    0
 *
 * size byte = if set then next data block is a the property size byte
 *             if clear then the 'word data' flag is checked and the property
 * has no size byte word data = if set then 2 data blocks follow if clear 1 data
 * block follows ignored = this flag is not used by the property manipulation
 * opcodes, it can be set to an arbitary value (note: this bit could be used to
 * increase the property size from 63 to 127 bytes) id = property identifier
    ...
    *obj_table_end = object_address - 1;
    ...
    while ((data = read_data_byte(obj_data_end)) != 0) {
        if ((unsigned int)header.version < V4)
            ...
            data_count =
                (unsigned int)read_data_byte(obj_data_end) & property_size_mask;
        else if (data & 0x40)
            data_count = 2;
        else
            data_count = 1;
...
#endif
{
    ...
    for (i = 1; (unsigned int)i <= obj_count; i++) {
        ...
        if ((unsigned int)read_data_byte(&address)) (void)decode_text(&address);
        tx_printf("\"\n");
...
#endif
{
    ...
    /* Address calculation is object table base + size of default properties
       area
...
#endif
{
    ...
    for (i = 1; i <= obj_count; i++) {
        ...
        if (parent == 0) print_object((int)i, 0);
    }
...
#endif
{
    ...
    while (obj) {
        ...
        if (child) print_object(child, depth + 1);
    }
