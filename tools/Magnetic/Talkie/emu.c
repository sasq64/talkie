...
*
* 2.0  080600 : [SM] Minor change to A0DF code for preventing mysteriousenu
*               crash in MW-Fish!
*
...
uint32_t dreg[8], areg[8], i_count, string_size, rseed = 0, pc, arg1i, mem_size;
uint16_t properties, fl_sub, fl_tab, fl_size, fp_tab, fp_size;
uint8_t zflag, nflag, cflag, vflag, byte1, byte2, regnr, admode, opsize;
uint8_t *arg1, *arg2, is_reversible, running = 0, tmparg[4] = {0, 0, 0, 0};
uint8_t lastchar = 0, version = 0, sd = 0;
uint8_t *decode_table, *restart = 0, *code = 0, *string = 0, *string2 = 0;
uint8_t *string3 = 0, *dict = 0;
uint8_t quick_flag = 0, gfx_ver = 0, *gfx_buf = 0, *gfx_data = 0;
uint8_t *gfx2_hdr = 0, *gfx2_buf = 0;
int8_t* gfx2_name = 0;
uint16_t gfx2_hsize = 0;
FILE* gfx_fp = 0;
uint8_t *snd_buf = 0, *snd_hdr = 0;
uint16_t snd_hsize = 0;
FILE* snd_fp = 0;

const int8_t undo_ok[] = "\n[Previous turn undone.]";
const int8_t undo_fail[] = "\n[You can't \"undo\" what hasn't been done!]";
uint32_t undo_regs[2][18], undo_pc, undo_size;
uint8_t *undo[2] = {0, 0}, undo_stat[2] = {0, 0};
uint16_t gfxtable = 0, table_dist = 0;
uint16_t v4_id = 0, next_table = 1;
...
uint16_t pos_table_size = 0;
uint16_t pos_table_count[MAX_POSITIONS];
struct ms_position pos_table[MAX_POSITIONS][MAX_ANIMS];
uint8_t* command_table = 0;
int16_t command_index = -1;
struct lookup anim_table[MAX_POSITIONS];
int16_t pos_table_index = -1;
int16_t pos_table_max = -1;
struct ms_position pos_array[MAX_FRAMES];
uint8_t anim_repeat = 0;
...
uint8_t* hint_contents = 0;
const int8_t no_hints[] = "[Hints are not available.]\n";
const int8_t not_supported[] = "[This function is not supported.]\n";
...
/* prototypes */
uint32_t read_reg(int, int);
void write_reg(int, int, uint32_t);
...
uint8_t* effective(uint32_t ptr)
{
    if ((version < 4) && (mem_size == 0x10000)) return &(code[ptr & 0xffff]);
    if (ptr >= mem_size) {
        ms_fatal("Outside memory experience");
        return code;
    }
#if defined(__MSDOS__) && defined(__BORLANDC__)
    return (uint8_t*)(((uint8_t huge*)code) + ptr);
#else
    return &(code[ptr]);
#endif
}

uint32_t read_l(uint8_t* ptr)
{
    return (uint32_t)((uint32_t)ptr[0] << 24 | (uint32_t)ptr[1] << 16 |
                      (uint32_t)ptr[2] << 8 | (uint32_t)ptr[3]);
}

uint16_t read_w(uint8_t* ptr)
{
    return (uint16_t)(ptr[0] << 8 | ptr[1]);
}

void write_l(uint8_t* ptr, uint32_t val)
{
    ptr[3] = (uint8_t)val;
    val >>= 8;
    ptr[2] = (uint8_t)val;
...
void write_w(uint8_t* ptr, uint16_t val)
{
    ptr[1] = (uint8_t)val;
    val >>= 8;
    ptr[0] = (uint8_t)val;
...
uint32_t read_l2(uint8_t* ptr)
{
    return ((uint32_t)ptr[1] << 24 | (uint32_t)ptr[0] << 16 |
            (uint32_t)ptr[3] << 8 | (uint32_t)ptr[2]);
}

uint16_t read_w2(uint8_t* ptr)
{
...
void ms_seed(uint32_t seed)
{
    rseed = seed;
}

uint32_t rand_emu(void)
{
...
void ms_freemem(void)
{
    ...
    if (gfx2_hdr) free(gfx2_hdr);
    if (gfx2_buf) free(gfx2_buf);
    ...
    lastchar = 0;
    if (hints) free(hints);
    if (hint_contents) free(hint_contents);
    hints = 0;
    hint_contents = 0;
    if (snd_hdr) free(snd_hdr);
...
uint8_t ms_is_running(void)
{
    return running;
}

uint8_t ms_is_magwin(void)
{
...
uint8_t init_gfx1(uint8_t* header)
{
#ifdef SAVEMEM
    uint32_t i;
#endif
...
#ifdef SAVEMEM
    for (i = 0; i < 128; i += 4)
        if (!read_l(gfx_data + i)) write_l(gfx_data + i, read_l(header + 4));
#else
    fclose(gfx_fp);
    gfx_fp = 0;
#endif
...
uint8_t init_gfx2(uint8_t* header)
{
    if (!(gfx_buf = malloc(MAX_PICTURE_SIZE))) {
        fclose(gfx_fp);
        gfx_fp = 0;
...
uint8_t init_snd(uint8_t* header)
{
...
{
    FILE* fp;
    uint8_t header[42], header2[8], header3[4];
    uint32_t i, dict_size, string2_size, code_size, dec;
    ...
    if (!name) {
        ...
        undo_stat[0] = undo_stat[1] = 0;
        ms_seed((uint32_t)time(0));
        if (!(fp = fopen(name, "rb"))) return 0;
        if ((fread(header, 1, 42, fp) != 42) ||
            (read_l(header) != 0x4d615363)) {
            fclose(fp);
            return 0;
        }
        ...
        if (undo_size == 0) undo_size = 8;

        sd =
            (uint8_t)((dict_size != 0L) ? 1 : 0); /* if (sd) => separate dict */

        if (!(code = malloc(mem_size)) || !(string2 = malloc(string2_size)) ||
            !(restart = malloc(undo_size)) ||
            (sd && !(dict = malloc(dict_size)))) {
            ms_freemem();
            fclose(fp);
            return 0;
        }
        ...
        if (string_size > MAX_STRING_SIZE) {
            ...
            if (fread(string3, 1, string_size - MAX_STRING_SIZE, fp) !=
    ...
    if (!name) return (uint8_t)(gfx_buf ? 2 : 1); /* Restarted */

    if (version == 4) {
        /* Try loading a hint file */
        FILE* hnt_fp;
        if (hntname && (hnt_fp = fopen(hntname, "rb"))) {
            if ((fread(&header3, 1, 4, hnt_fp) == 4) &&
                (read_l(header3) == 0x4D614874)) {
                uint8_t buf[8];
                uint16_t i, j, blkcnt, elcnt, ntype, elsize, conidx;
...
uint8_t is_blank(uint16_t line, uint16_t width)
{
    int32_t i;

    for (i = line * width; i < (line + 1) * width; i++)
        if (gfx_buf[i]) return 0;
    return 1;
}

uint8_t* ms_extract1(uint8_t pic, uint16_t* w, uint16_t* h, uint16_t* pal)
{
    uint8_t *decode_table, *data, bit, val, *buffer;
    uint16_t tablesize, count;
    uint32_t i, j, datasize, upsize, offset;
    ...
    w[0] = (uint16_t)(read_w(buffer + 4) - read_w(buffer + 2));
    h[0] = read_w(buffer + 6);
    ...
    free(buffer);
#endif
    for (; h[0] > 0 && is_blank((uint16_t)(h[0] - 1), w[0]); h[0]--)
        ;
    for (i = 0; h[0] > 0 && is_blank((uint16_t)i, w[0]); h[0]--, i++)
        ;
    return gfx_buf + i * w[0];
}

int16_t find_name_in_header(int8_t* name, uint8_t upper)
{
    int16_t header_pos = 0;
    int8_t pic_name[8];
    uint8_t i;
    ...
    if (upper) {
        for (i = 0; i < 8; i++)
            pic_name[i] = (int8_t)toupper(pic_name[i]);
    }

    while (header_pos < gfx2_hsize) {
        int8_t* hname = (int8_t*)(gfx2_hdr + header_pos);
        if (strncmp(hname, pic_name, 6) == 0) return header_pos;
        header_pos += 16;
    }
    return -1;
}
...
{
    uint32_t i, x, y, bit_x, mask, ywb, yw, value, values[4];
    ...
    for (y = 0; y < pic->height; y++) {
        ...
        for (x = 0; x < pic->width; x++) {
            if ((x % 8) == 0) {
                for (i = 0; i < 4; i++)
                    values[i] =
                        pic->data[ywb + (x / 8) + (pic->plane_step * i)];
            }
            ...
            gfx_buf[yw + x] = (uint8_t)value;
        }
    }
}

uint8_t* ms_extract2(int8_t* name, uint16_t* w, uint16_t* h, uint16_t* pal,
                     uint8_t* is_anim)
{
    struct picture main_pic;
    uint32_t offset = 0, length = 0, i;
    int16_t header_pos = -1;
#ifndef NO_ANIMATION
    uint8_t* anim_data;
    uint32_t j;
#endif

    if (is_anim != 0) *is_anim = 0;
    gfx2_name = name;
...
#endif
    if (header_pos < 0) header_pos = find_name_in_header(name, 0);
    ...
    if (offset != 0) {
        ...
        *h = main_pic.height;
        ...
        if ((anim_data[0] != 0xD0) || (anim_data[1] != 0x5E)) {
            uint8_t* current;
            uint16_t frame_count, command_count;
            ...
            for (i = 0; i < frame_count; i++) {
                ...
                anim_frame_table[i].wbytes =
                    (uint16_t)(anim_frame_table[i].data_size /
                               anim_frame_table[i].height);
                anim_frame_table[i].plane_step =
                    (uint16_t)(anim_frame_table[i].wbytes / 4);
                anim_frame_table[i].mask = (uint8_t*)0;
                ...
                /* Get the mask */
                if ((value1 == anim_frame_table[i].width) &&
                    (value2 == anim_frame_table[i].height)) {
                    uint16_t skip;

                    anim_frame_table[i].mask = (uint8_t*)(current + 4);
...
#    endif
            for (i = 0; i < pos_table_size; i++) {
                ...
                for (j = 0; j < pos_table_count[i]; j++) {
                    ...
                    current += 8;
#    ifdef LOGGFX_EXT
                    out2("Position entry: Table: %d  Entry: %d  X: %d Y: %d "
                         "Frame: %d\n",
                         i, j, pos_table[i][j].x, pos_table[i][j].y,
                         pos_table[i][j].number);
#    endif
                }
            }
...
uint8_t* ms_extract(uint32_t pic, uint16_t* w, uint16_t* h, uint16_t* pal,
                    uint8_t* is_anim)
{
    if (is_anim) *is_anim = 0;

    if (gfx_buf) {
        switch (gfx_ver) {
        case 1:
            return ms_extract1((uint8_t)pic, w, h, pal);
        case 2:
            return ms_extract2((int8_t*)(code + pic), w, h, pal, is_anim);
        }
    }
    return 0;
}
...
uint8_t ms_animate(struct ms_position** positions, uint16_t* count)
{
    ...
    if ((gfx_buf == 0) || (gfx2_buf == 0) || (gfx_ver != 2)) return 0;
...
#    endif
        ...
        if (got_anim == 0) {
            uint8_t command = command_table[command_index];
            command_index++;
...
#    endif
                ...
                anim_table[ttable - 1].flag =
                    (int16_t)(command_table[command_index] - 1);
                command_index++;
                anim_table[ttable - 1].count =
                    (int16_t)(command_table[command_index] - 1);
                command_index++;
...
#    endif
            case 0x04:
#    ifdef LOGGFX_EXT
                out2("Unknown Command: %d Prop1: %d  Prop2: %d WARNING:not "
                     "parsed\n",
...
uint8_t ms_anim_is_repeating(void)
{
#ifndef NO_ANIMATION
    return anim_repeat;
#else
...
int16_t find_name_in_sndheader(int8_t* name)
{
    int16_t header_pos = 0;

    while (header_pos < snd_hsize) {
        int8_t* hname = (int8_t*)(snd_hdr + header_pos);
        if (strcmp(hname, name) == 0) return header_pos;
        header_pos += 18;
    }
    return -1;
}

uint8_t* sound_extract(int8_t* name, uint32_t* length, uint16_t* tempo)
{
    uint32_t offset = 0;
    int16_t header_pos = -1;

    if (header_pos < 0) header_pos = find_name_in_sndheader(name);
    if (header_pos < 0) return 0;
    ...
    *length = read_l(snd_hdr + header_pos + 14);
    ...
    if (offset != 0) {
        if (!snd_buf) return 0;
        if (fseek(snd_fp, offset, SEEK_SET) < 0) return 0;
        if (!fread(snd_buf, (int)(*length), 1, snd_fp)) return 0;
        return snd_buf;
    }
    return 0;
}
...
void ms_status(void)
{
    ...
    fprintf(stderr, "\nPC=%5.5lx ZCNV=%d%d%d%d - %ld instructions\n", (long)pc,
            zflag & 1, cflag & 1, nflag & 1, vflag & 1, (long)i_count);
}
...
uint8_t* reg_align(uint8_t* ptr, uint8_t size)
{
    if (size == 1) ptr += 2;
    if (size == 0) ptr += 3;
    return ptr;
...
void write_reg(int i, int s, uint32_t val)
{
    uint8_t* ptr;
    ...
    switch (s) {
    case 0:
        reg_align(ptr, 0)[0] = (uint8_t)val;
        break;
    case 1:
        write_w(reg_align(ptr, 1), (uint16_t)val);
        break;
    default:
        write_l(ptr, val);
        break;
...
void char_out(uint8_t c)
{
    static uint8_t big = 0, period = 0, pipe = 0;
    ...
    if (version < 4) {
        ...
        if ((c == 0x20) && (lastchar == 0x0a)) return;
        if ((c == 0x2e) || (c == 0x3f) || (c == 0x21) || (c == 0x0a))
            big = 1;
        else if (c == 0x22)
            big = 0;
...
void set_info(uint8_t b)
{
    regnr = (uint8_t)(b & 0x07);
    admode = (uint8_t)((b >> 3) & 0x07);
    opsize = (uint8_t)(b >> 6);
}
...
void set_arg1(void)
{
    ...
    switch (admode) {
    case 0:
        arg1 = reg_align((uint8_t*)&dreg[regnr], opsize); /* Dx */
        is_reversible = 0;
#ifdef LOGEMU
        out(" d%.1d", regnr);
#endif
        break;
    case 1:
        arg1 = reg_align((uint8_t*)&areg[regnr], opsize); /* Ax */
        is_reversible = 0;
#ifdef LOGEMU
        out(" a%.1d", regnr);
#endif
    ...
    case 4:
        ...
        break;
    case 5: {
        int16_t i = (int16_t)read_w(effective(pc));
    ...
    case 6:
...
#endif
        arg1i = read_reg(regnr + 8, 2) + (int8_t)byte2;
#ifdef LOGEMU
        if ((byte1 >> 4) > 8)
            out("a%.1d", (byte1 >> 4) - 8);
        else
        ...
        if (byte1 & 0x08) {
            ...
            out(".l)");
#endif
            arg1i += (int32_t)read_reg((byte1 >> 4), 2);
        } else {
#ifdef LOGEMU
            out(".w)");
#endif
            arg1i += (int16_t)read_reg((byte1 >> 4), 1);
        }
        byte1 = tmp[0];
        byte2 = tmp[1];
        break;
    case 7: /* specials */
        ...
        case 1:
            ...
            break;
        case 2:
            arg1i = (int16_t)read_w(effective(pc)) + pc; /* $xxxx(PC) */
            pc += 2;
#ifdef LOGEMU
            out(" %.4X(pc)", arg1i);
#endif
...
#endif
            if (l1c & 0x08)
                arg1i = pc + (int32_t)read_reg((l1c >> 4), 2);
            else
                arg1i = pc + (int16_t)read_reg((l1c >> 4), 1);
            l1c = effective(pc)[1];
            pc += 2;
            arg1i += (int8_t)l1c;
            break;
...
void set_arg2_nosize(int use_dx, uint8_t b)
{
    if (use_dx)
        arg2 = (uint8_t*)dreg;
    else
...
void set_arg2(int use_dx, uint8_t b)
{
...
void branch(uint8_t b)
{
    if (b == 0)
        pc += (int16_t)read_w(effective(pc));
    else
        pc += (int8_t)b;
#ifdef LOGEMU
    out(" %.4X", pc);
...
void do_add(uint8_t adda)
{
    if (adda) {
        if (opsize == 0) write_l(arg1, read_l(arg1) + (int8_t)arg2[0]);
        if (opsize == 1) write_l(arg1, read_l(arg1) + (int16_t)read_w(arg2));
        if (opsize == 2) write_l(arg1, read_l(arg1) + (int32_t)read_l(arg2));
    } else {
        cflag = 0;
        if (opsize == 0) {
            arg1[0] += arg2[0];
        ...
        if (opsize == 1) {
            write_w(arg1, (uint16_t)(read_w(arg1) + read_w(arg2)));
...
void do_eor(void)
{
    ...
    if (opsize == 1) write_w(arg1, (uint16_t)(read_w(arg1) ^ read_w(arg2)));
...
void do_bop(uint8_t b, uint8_t a)
{
#ifdef LOGEMU
...
void check_movem(void)
{
    ...
    out("movem");
#endif
    set_info((uint8_t)(byte2 - 0x40));
    read_word();
    for (l1c = 0; l1c < 8; l1c++) {
        if (byte2 & 1 << l1c) {
            set_arg1();
            if (opsize == 2) write_l(arg1, read_reg(15 - l1c, 2));
            if (opsize == 1) write_w(arg1, (uint16_t)read_reg(15 - l1c, 1));
        }
    }
    for (l1c = 0; l1c < 8; l1c++) {
        if (byte1 & 1 << l1c) {
            ...
            if (opsize == 1) write_w(arg1, (uint16_t)read_reg(7 - l1c, 1));
        }
    }
}
...
{
    uint16_t dtab, doff, output, output_bak, bank, word, output2;
    uint16_t tmp16, i, obj_adj, adjlist, adjlist_bak;
    uint8_t c, c2, c3, flag, matchlen, longest, flag2;
    uint8_t restart = 0, accept = 0;
    ...
    dtab = (uint16_t)read_reg(8 + 5, 1); /* used by version>0 */
    output = (uint16_t)read_reg(8 + 2, 1);
    write_reg(8 + 5, 1, read_reg(8 + 6, 1));
    doff = (uint16_t)read_reg(8 + 3, 1);
    adjlist = (uint16_t)read_reg(8 + 0, 1);

    bank = (uint16_t)read_reg(6, 0); /* l2d */
    flag = 0;                        /* l2c */
    word = 0;                        /* l26 */
    matchlen = 0;                    /* l2e */
    longest = 0;                     /* 30e2 */
    ...
    while ((c = sd ? dict[doff] : effective(doff)[0]) != 0x81) {
            ...
            if (c2 == c) {
                ...
                if ((!c) || (c == 0x20) || (c == 0x27) ||
                    ...
                    if (c == 0x27) {
                        write_reg(8 + 6, 1, read_reg(8 + 6, 1) + 1);
                        write_reg(0, 1,
                                  0x200 + effective(read_reg(8 + 6, 1))[0]);
                    }
                    if ((version < 4) || (c3 != 0xa0)) accept = 1;
                } else
                    restart = 1;
        ...
        if (accept) {
            ...
            effective(read_reg(8 + 2, 1))[1] = (uint8_t)bank;
    ...
    if (version) {
        ...
        while ((c = effective(output)[1]) != 0xff) {
            if (c == 0x0b) {
                if (sd)
                    tmp16 =
                        read_w(&dict[dtab + read_w(effective(output + 2)) * 2]);
                else
                    tmp16 = read_w(
                        effective(dtab + read_w(effective(output + 2)) * 2));
                effective(output)[1] = tmp16 & 0x1f;
                write_w(effective(output + 2), (uint16_t)(tmp16 >> 5));
            }
            output += 4;
        }
        output = output_bak;
    ...
    /* l1c = adjlist_bak, 333C = i,      l2d = bank,    l2c = flag, l30e3 =
     * flag2 */
    ...
    while ((bank = effective(output2)[1]) != 0xff) {
        obj_adj = (uint16_t)read_reg(
            8 + 1, 1);      /* A1.W - obj_adj, ie. adjs for this word */
        ...
        if (!read_reg(1, 0)) {
            ...
            effective(output)[1] = (uint8_t)bank;
...
void do_findprop(void)
{
    ...
    if ((version > 2) && ((read_reg(0, 1) & 0x3fff) > fp_size)) {
        tmp = (uint16_t)(((fp_size - (read_reg(0, 1) & 0x3fff)) ^ 0xffff) << 1);
        tmp += fp_tab;
...
{
    static uint32_t offset_bak;
    static uint8_t mask_bak;
    uint8_t c, b, mask;
    uint16_t ptr;
    uint32_t offset;
...
void output_number(uint16_t number)
{
    uint16_t tens = number / 10;

    if (tens > 0) ms_putchar('0' + tens);
    number -= (tens * 10);
    ms_putchar('0' + number);
}

uint16_t output_text(const char* text)
{
    uint16_t i;
...
int16_t hint_input(void)
{
    ...
    if (c1 == 1) return -1; /* New game loaded, abort hints */
...
uint16_t show_hints_text(struct ms_hint* hints, uint16_t index)
{
    uint16_t i = 0, j = 0;
    int16_t input;
    ...
    while (1) {
        ...
        case 1: /* folders */
            ...
            case -4: /* Show parent hint list */
                if (hint->parent != 0xffff) return 0;
            default:
...
void do_line_a(void)
{
    uint8_t l1c;
    int8_t* str;
    uint16_t ptr, ptr2, tmp16, dtype;
    uint32_t a1reg, tmp32;
    ...
    if ((byte2 < 0xdd) || (version < 4 && byte2 < 0xe4) ||
        ...
        case 2: /* A0DF */
            ...
            case 10: /* Open window commands */
                ...
                case 4: /* Help/Hints */
                    if (hints != 0) {
                        if (ms_showhints(hints) == 0) show_hints_text(hints, 0);
                    } else
            ...
            case 13: /* Music */
                ...
                default: /* play music */
...
#endif
                    {
                        ...
                        uint16_t tempo = 0;
                        uint8_t* midi =
                            sound_extract(code + a1reg + 3, &length, &tempo);
                        if (midi != NULL) ms_playmusic(midi, length, tempo);
                    }
                    break;
                }
                break;
        ...
        case 6: /* A0E3 */
            if (read_reg(1, 2) == 0) {
                if ((version < 4) || (read_reg(6, 2) == 0)) ms_showpic(0, 0);
            }
            /* printf("\nMoves: %u\n",read_reg(0,1)); */
            break;
        ...
        case 12: /* A0E9 [3083 - j] */
            ptr = (uint16_t)read_reg(8 + 0, 1);
            ptr2 = (uint16_t)read_reg(8 + 1, 1);
            do {
                l1c = dict[ptr2++];
                effective(ptr++)[0] = l1c;
            } while ((l1c & 0x80) == 0);
        ...
        case 14: /* A0EB [3037 - j] */
            dict[read_reg(8 + 1, 1)] = (uint8_t)read_reg(1, 0);
            break;
        ...
        case 18: /* printer A0EF */
            break;
        case 19:
            ms_showpic(read_reg(0, 0),
                       (uint8_t)read_reg(1, 0)); /* Do_picture(D0) A0F0 */
            break;
        case 20:
            ptr = (uint16_t)read_reg(8 + 1, 1); /* A1=nth_string(A1,D0) A0F1 */
            tmp32 = read_reg(0, 1);
            while (tmp32-- > 0) {
        ...
        case 22:
            char_out((uint8_t)read_reg(1, 0)); /* A0F3 */
            break;

        case 23: /* D7=Save_(filename A0) D1 bytes starting from A1  A0F4 */
            str = (version < 4) ? (int8_t*)effective(read_reg(8 + 0, 1)) : 0;
            write_reg(7, 0,
                      ms_save_file(str, effective(read_reg(8 + 1, 1)),
                                   (uint16_t)read_reg(1, 1)));
            break;

        case 24: /* D7=Load_(filename A0) D1 bytes starting from A1  A0F5 */
            str = (version < 4) ? (int8_t*)effective(read_reg(8 + 0, 1)) : 0;
            write_reg(7, 0,
                      ms_load_file(str, effective(read_reg(8 + 1, 1)),
                                   (uint16_t)read_reg(1, 1)));
            break;

        case 25: /* D1=Random(0..D1-1) [3748] A0F6 */
            l1c = (uint8_t)read_reg(1, 0);
            write_reg(1, 1, rand_emu() % (l1c ? l1c : 1));
            break;

        case 26: /* D0=Random(0..255) [3742] A0F7 */
            tmp16 = (uint16_t)rand_emu();
        ...
        case 28: /* Z,D0=Get_inventory_item(D0) [2a9e] A0F9 */
            ...
            do {
                ...
                do {
                    do_findprop();
                    ptr2 = (uint16_t)read_reg(8 + 0, 1); /* object properties */
        ...
        case 29: /* [2b18] A0FA */
            ptr = (uint16_t)read_reg(8, 1);
            do {
                if (read_reg(5, 0)) {
                    l1c = ((uint32_t)((read_w(effective(ptr)) & 0x3fff)) ==
                           read_reg(2, 1));
                } else {
        ...
        case 32: /* Set properties pointer from A0 [2b7b] A0FD */
            properties = (uint16_t)read_reg(8 + 0, 1);
            if (version > 0) fl_sub = (uint16_t)read_reg(8 + 3, 1);
            if (version > 1) {
                fl_tab = (uint16_t)read_reg(8 + 5, 1);
                fl_size = (uint16_t)read_reg(7, 1) + 1;
                /* A3 [routine], A5 [table] and D7 [table-size] */
            }
            if (version > 2) {
                fp_tab = (uint16_t)read_reg(8 + 6, 1);
                fp_size = (uint16_t)read_reg(6, 1);
            }
            break;
...
uint8_t ms_rungame(void)
{
    ...
    case 0x17:
        ...
        out("move.l");
#endif
        set_info((uint8_t)((byte2 & 0x3f) | 0x80));
        set_arg1();
        swap_args();
        l1c = (byte1 >> 1 & 0x07) | (byte2 >> 3 & 0x18) | (byte1 << 5 & 0x20);
        set_info((uint8_t)(l1c | 0x80));
        set_arg1();
        do_move();
        break;
    ...
    case 0x1f:
        ...
        out("move.w");
#endif
        set_info((uint8_t)((byte2 & 0x3f) | 0x40));
        set_arg1();
        swap_args();
        l1c = (byte1 >> 1 & 0x07) | (byte2 >> 3 & 0x18) | (byte1 << 5 & 0x20);
        set_info((uint8_t)(l1c | 0x40));
        set_arg1();
        do_move();
        break;
    ...
    case 0x22:
        if (byte1 == 0x44) {
            ...
            if ((byte2 & 0xc0) == 0xc0) {
                ...
                if (opsize == 1) {
                    write_w(arg1, (uint16_t)(-1 * read_w(arg1)));
    ...
    case 0x24:
        if (byte1 == 0x48) {
            if ((byte2 & 0xf8) == 0x40) {
                ...
                out("pea");
#endif
                set_info((uint8_t)((byte2 & 0x3f) | 0x80)); /* PEA */
                set_arg1();
                if (is_reversible)
                    push(arg1i);
                else
    ...
    case 0x27:
        if (byte1 == 0x4e) {
            ...
            if (byte2 == 0x75) {
                ...
                out("jsr");
#endif
                set_info((uint8_t)(byte2 | 0xc0)); /* indir JSR */
                set_arg1();
                push(pc);
                if (is_reversible)
                    pc = arg1i;
        ...
        if ((byte2 & 0xc0) == 0xc0) {
            ...
            if (admode == 1) {
                ...
                if (condition(byte1) == 0) {
                    arg1 = (arg1 - (uint8_t*)areg) + (uint8_t*)dreg - 1;
                    write_w(arg1, (uint16_t)(read_w(arg1) - 1));
            ...
            arg2 = reg_align(tmparg, opsize);
            {
                ...
                switch (opsize) {
                case 0:
                    outnum = (int8_t)arg2[0];
                    break;
                case 1:
                    outnum = (int16_t)read_w(arg2);
                    break;
                case 2:
                    outnum = (int32_t)read_l(arg2);
                    break;
                }
#endif
                if ((byte1 & 0x01) == 1) {
    ...
    case 0x3f:
        ...
        out("moveq");
#endif
        arg1 = (uint8_t*)&dreg[byte1 >> 1 & 0x07];
        if (byte2 > 127)
...
#endif
        if ((byte2 & 0xc0) == 0xc0) {
            set_info((uint8_t)(byte2 & 0xbf)); /* OP Dx */
            set_arg1();
            l1c = 1; /* steps=1 */
            byte2 = (byte1 >> 1) & 0x03;
        } else {
            set_info((uint8_t)(byte2 & 0xc7));
            set_arg1();
            if ((byte2 & 0x20) == 0) {
                /* immediate */
                l1c = (byte1 >> 1) & 0x07;
                ...
                l1c = (uint8_t)read_reg(byte1 >> 1 & 0x07, 0);
            }
            byte2 = (byte2 >> 3) & 0x03;
        }
        if ((byte1 & 0x01) == 0) {
            ...
            while (l1c-- > 0) {
                ...
                if (opsize == 1) {
                    ...
                    if (cflag && (byte2 == 3))
                        write_w(arg1,
                                (uint16_t)(read_w(arg1) | ((uint16_t)1 << 15)));
                }
                if (opsize == 2) {
                    cflag = read_l(arg1) & 0x01 ? 0xff : 0;
                    write_l(arg1, read_l(arg1) >> 1);
                    if (cflag && (byte2 == 3))
                        write_l(arg1, read_l(arg1) | ((uint32_t)1 << 31));
                }
            }
        } else {
            /* left */
            while (l1c-- > 0) {
                ...
                if (opsize == 1) {
                    cflag = read_w(arg1) & ((uint16_t)1 << 15) ? 0xff : 0;
                    write_w(arg1, (uint16_t)(read_w(arg1) << 1));
                    if (cflag && (byte2 == 3))
                        write_w(arg1, (uint16_t)(read_w(arg1) | 0x01));
                }
                if (opsize == 2) {
                    cflag = read_l(arg1) & ((uint32_t)1 << 31) ? 0xff : 0;
                    write_l(arg1, read_l(arg1) << 1);
                    if (cflag && (byte2 == 3))
    ...
    case 0x7f:
        ...
        if (version == 0) {
            /* hardcoded jump */
            char_out(l1c = (uint8_t)read_reg(1, 0));
        } else if (version == 1) {
            ...
            if (ptr >= fl_size) {
                ...
                pc = (uint32_t)ptr + (int16_t)read_w(effective(ptr));
            } else {
                push(pc);
                pc = fl_sub;
            }
