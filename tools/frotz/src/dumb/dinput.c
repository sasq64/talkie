 ...
 * Temporarily disables canonical mode to allow single-char input. */
static int read_single_char(void)
{
	...
	if (tcgetattr(STDIN_FILENO, &old_tios) < 0) {
		/* If tcgetattr fails, fall back to line input */
		return xgetchar();
	}

	/* Create new settings: disable canonical mode and echo */
	new_tios = old_tios;
	new_tios.c_lflag &= ~(ICANON | ECHO);
	new_tios.c_cc[VMIN] = 1;   /* Block until at least 1 char is available */
	new_tios.c_cc[VTIME] = 0;  /* No timeout */

	/* Apply new settings */
	if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &new_tios) < 0) {
		/* If tcsetattr fails, fall back to line input */
		return xgetchar();
	}
...
zchar os_read_key (int timeout, bool show_cursor)
{
	...
	if (read_key_buffer[0] == '\0') {
		/* For single character input, read directly without line buffering */
		dumb_show_screen(show_cursor);
		if (show_cursor)
			dumb_show_prompt(show_cursor, ')');
		fflush(stdout);
